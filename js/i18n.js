/**
 * Internationalisation — translations + t() helper.
 * Exposes: window.currentLang, window.t(), window.applyLang()
 */

window.currentLang = 'zh';

window.TRANSLATIONS = {
  en: {
    tabTitle: 'FB Social Graph Visualiser',
    title: 'FB Social Graph Visualiser',
    subtitle: "See how Facebook's ML ranks your friends",
    step1: 'Go to <a href="https://www.facebook.com" target="_blank" rel="noopener noreferrer"><strong>facebook.com</strong></a>',
    step2: 'On desktop, press <strong>Ctrl+U</strong> (<strong>⌘+U</strong> on Mac) to view source',
    step3: '<strong>Ctrl+A</strong> select all, <strong>Ctrl+C</strong> copy, then paste below',
    mobileBanner: "This tool needs a desktop browser — viewing Facebook's page source isn't available on phones.",
    sourceInputLabel: 'Facebook page source',
    graphHint: 'Tap a node to open the profile · drag to pin · pinch or scroll to zoom · press / to search',
    placeholder: 'Paste your Facebook page source here (Ctrl+V)...',
    btnAnalyse: 'Analyse',
    btnParsing: 'Parsing...',
    btnClear: 'Clear',
    btnThemeDark: 'Dark',
    btnThemeLight: 'Light',
    privacy: 'Your data never leaves this page. All parsing happens locally in your browser.',
    explainerTitle: 'What do these numbers mean?',
    scoreDt: 'Score (0–100)',
    scoreDd: "Facebook's ML model assigns each friend a <strong>base affinity</strong> (feature 16173) reflecting how likely you are to interact with them. Normalised to 0–100 relative to your own social graph, where 100 = your closest friend.",
    rawDt: 'Raw Base',
    rawDd: "The original affinity value from Facebook's payload. The range is narrow (~0.389–0.414) because it's a probability-like weight, but small differences matter at scale.",
    activeDt: 'Active',
    activeDd: 'Detects a structural +0.5 engagement bonus in the multiplier (feature 16174). "YES" means Facebook considers you actively engaging with this person — messaging, reacting, or visiting their profile.',
    errorParse: 'Parsing failed: ',
    errorEmpty: 'No profile data found. Make sure you pasted the full page source from facebook.com (Ctrl+U).',
    tableTitle: n => `Full Ranked List (${n} profiles)`,
    statTopLabel: 'Your closest friend',
    statActiveLabel: 'Actively engaged',
    statActiveVal: (nActive, n) => `${nActive} of ${n}`,
    statMutualLabel: 'Mutual best-friends',
    statMutualVal: n => `${n}`,
    chartDonut: 'Active vs Inactive',
    chartDonutL: ['Active (+0.5 bonus)', 'Inactive'],
    colRank: 'Rank',
    colName: 'Name',
    colRaw: 'Raw Base',
    colScore: 'Score',
    colActive: 'Active',
    activeYes: 'YES',
    activeNo: 'NO',
    graphTitle: 'Affinity Orbit — Top 50 Friends',
    graphLegendActive: 'Active',
    graphLegendInact: 'Inactive',
    graphLegendImport: 'Imported edges',
    graphCenter: 'You',
    graphHoverScore: 'Score',
    searchPlaceholder: 'Search friends...',
    searchCount: n => `${n} result${n === 1 ? '' : 's'}`,
    btnExport: 'Export CSV',
    btnImport: "Import Friend's CSV",

    btnShare: 'Share Graph',
    importModalTitle: "Whose friend list is this?",
    importModalDesc: "Select the friend who exported this CSV. Edges will be drawn from them to their friends who also appear in your top 50.",
    importModalConfirm: 'Confirm',
    importModalCancel: 'Cancel',
    importModalNoData: 'Please analyse your Facebook source first before importing.',
    step4: '(Optional) Ask friends to use this tool too, export their CSV, and import it below to see mutual connections',
    toastCopied: 'Copied to clipboard — paste it anywhere!',
    toastSaved: 'Image saved to downloads.',
    toastFailed: 'Could not capture graph.',
    toastImported: n => `Imported ${n} edge${n === 1 ? '' : 's'} — graph updated.`,
    toastImportErr: 'Could not parse CSV. Make sure it was exported from this app.',
    toastImportNoEdges: 'No matching friends found in your top 50. No edges added.',
    contributorsLabel: 'Imported from:',
    removeContributor: name => `Remove edges imported from ${name}`,
    sampleCsvSummary: "What's the file format?",
    sampleCsvIntro: "Two example rows from this app's CSV — same format your friends will export:",
  },
  zh: {
    tabTitle: 'FB 社交圖譜視覺化',
    title: 'FB 社交圖譜視覺化',
    subtitle: 'Facebook 的 ML 如何排名你的朋友',
    step1: '前往 <a href="https://www.facebook.com" target="_blank" rel="noopener noreferrer"><strong>facebook.com</strong></a>',
    step2: '在電腦上按 <strong>Ctrl+U</strong>（Mac 為 <strong>⌘+U</strong>）查看網頁原始碼',
    step3: '<strong>Ctrl+A</strong> 全選，<strong>Ctrl+C</strong> 複製後貼上到下方',
    mobileBanner: '本工具需要桌機瀏覽器 — 手機無法查看 Facebook 的網頁原始碼。',
    sourceInputLabel: 'Facebook 網頁原始碼',
    graphHint: '點擊節點開啟個人檔案 · 拖曳固定位置 · 雙指縮放查看 · 按 / 鍵搜尋',
    placeholder: '將 Facebook 網頁原始碼貼到此處（Ctrl+V）...',
    btnAnalyse: '分析',
    btnParsing: '解析中...',
    btnClear: '清除',
    btnThemeDark: '深色',
    btnThemeLight: '淺色',
    privacy: '資料不會離開此頁面，所有解析皆在瀏覽器本地進行。',
    explainerTitle: '這些數字代表什麼？',
    scoreDt: '分數（0–100）',
    scoreDd: 'Facebook 的 ML 模型為每位朋友分配一個<strong>基礎親密度</strong>（特徵 16173），反映你與他們互動的可能性。相對於你的社交圖譜正規化至 0–100，100 分代表你最親近的朋友。',
    rawDt: '原始基礎值',
    rawDd: '直接來自 Facebook 資料的原始值，範圍狹窄（約 0.389–0.414），因為這是一種概率權重，但微小差異在排名上影響顯著。',
    activeDt: '活躍',
    activeDd: '偵測乘數（特徵 16174）中的 +0.5 結構性加分。「是」代表 Facebook 認為你正在積極與此人互動——傳訊息、按讚或瀏覽其個人檔案。',
    errorParse: '解析失敗：',
    errorEmpty: '找不到好友資料，請確認你貼上的是 facebook.com 完整頁面原始碼（Ctrl+U）。',
    tableTitle: n => `完整排名（${n} 位好友）`,
    statTopLabel: '最近好友',
    statActiveLabel: '活躍互動中',
    statActiveVal: (nActive, n) => `${nActive} / ${n}`,
    statMutualLabel: '雙向最親密',
    statMutualVal: n => `${n}`,
    chartDonut: '活躍 vs 非活躍',
    chartDonutL: ['活躍（+0.5 加成）', '非活躍'],
    colRank: '排名',
    colName: '姓名',
    colRaw: '原始值',
    colScore: '分數',
    colActive: '活躍',
    activeYes: '是',
    activeNo: '否',
    graphTitle: '親密軌道圖 — 前 50 名好友',
    graphLegendActive: '活躍',
    graphLegendInact: '非活躍',
    graphLegendImport: '匯入的連線',
    graphCenter: '你',
    graphHoverScore: '分數',
    searchPlaceholder: '搜尋好友...',
    searchCount: n => `${n} 筆結果`,
    btnExport: '匯出 CSV',
    btnImport: '匯入好友的 CSV',
    btnExportEdges: '匯出邊列表',
    btnShare: '分享圖表',
    importModalTitle: '這是誰的好友名單？',
    importModalDesc: '選擇匯出此 CSV 的好友。系統會畫出從該好友到你們共同好友（前 50 名中）的連線。',
    importModalConfirm: '確認',
    importModalCancel: '取消',
    importModalNoData: '請先分析你的 Facebook 原始碼，再匯入好友的 CSV。',
    step4: '（選用）請朋友也用此工具，匯出他們的 CSV，匯入後可看到共同好友之間的連線',
    toastCopied: '已複製到剪貼簿——貼到任何地方分享！',
    toastSaved: '圖片已儲存。',
    toastFailed: '無法擷取圖表。',
    toastImported: n => `已匯入 ${n} 條邊——圖表已更新。`,
    toastImportErr: '無法解析 CSV，請確認是由本工具匯出的檔案。',
    toastImportNoEdges: '在你的前 50 名中未找到匹配的好友，未新增任何邊。',
    contributorsLabel: '已匯入：',
    removeContributor: name => `移除來自 ${name} 的匯入連線`,
    sampleCsvSummary: '檔案格式長怎樣？',
    sampleCsvIntro: '這是本工具匯出的兩列範例 — 你的朋友匯出的檔案會是相同格式：',
  },
};

window.t = function (key, ...args) {
  const val = TRANSLATIONS[currentLang][key];
  return typeof val === 'function' ? val(...args) : (val ?? key);
};

window.applyLang = function () {
  document.documentElement.lang = currentLang === 'zh' ? 'zh-TW' : 'en';
  document.title = t('tabTitle');
  document.querySelectorAll('[data-i18n]').forEach(el => {
    const key = el.dataset.i18n;
    if (TRANSLATIONS[currentLang][key] !== undefined) el.innerHTML = t(key);
  });
  document.querySelectorAll('[data-i18n-placeholder]').forEach(el => {
    el.placeholder = t(el.dataset.i18nPlaceholder);
  });
};
